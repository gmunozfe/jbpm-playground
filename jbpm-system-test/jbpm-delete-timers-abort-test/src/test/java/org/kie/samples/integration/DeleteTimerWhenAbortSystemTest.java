...
class DeleteTimerWhenAbortSystemTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(DeleteTimerWhenAbortSystemTest.class);
    
    protected static Map<String, String> args = new HashMap<>();
    protected static final String bindDb = System.getProperty("noPatch")!=null? "target/postgresql":"src/test/resources/postgresql/patched";
    ...
    public static PostgreSQLContainer<?> postgreSQLContainer = new PostgreSQLContainer<>(System.getProperty("org.kie.samples.image.postgresql","postgres:latest"))
                                        ...
                                        .withFileSystemBind(bindDb, "/docker-entrypoint-initdb.d", BindMode.READ_ONLY)
    ...
    @ParameterizedTest
    @CsvSource({"timer-test.TimerProcess,0"})
    void testEJBTimerWithRollback(String processId, int expectedTimersAfterRollback) throws InterruptedException, SQLException {
    	...
    	map.put("timerDuration","2m");
        Long processInstanceId = processClient.startProcess(containerId, processId, map);
